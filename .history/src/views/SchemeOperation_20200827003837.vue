    ...
    schemes: {
      handler(val, oldVal) {
        val.map((o) => {
          ...
          // 根据方案金额模式分别发送下注命令
          if (o.isAllConnect) {
            console.log('o.sites:', o.sites)
            o.sites.map((s) => {
              const schemeIndex = val.findIndex((v) => v.schemeId === o.schemeId)
              const siteIndex = o.sites.findIndex((site) => site.siteId === s.siteId)
              console.log('new code:', s.gmcode)
              console.log('old code:', oldVal[schemeIndex].sites[siteIndex].gmcode)

              if (s.gmcode !== oldVal[schemeIndex].sites[siteIndex].gmcode) {
                console.log('s.gmcode:', s.gmcode)
                console.log('old.gmcode:', oldVal[schemeIndex].sites[siteIndex].gmcode)
    ...
    socket() {
      ...
      agInfo.map(async (agConfig) => {
        ...
        const result = await loginRoom(
          ...
          (gmcode) => {
            this.$set(site, 'gmcode', gmcode)
            console.log('socket code', gmcode)
            console.log('socket site', site)
            console.log('socket schemes', this.schemes)
          },
          (userEncryptKey) => {
