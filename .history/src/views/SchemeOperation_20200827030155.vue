...
export default {
  ...
  watch: {
    ...
    schemes: {
      handler(val, oldVal) {
        val.map((o) => {
          ...
          const platformWs = o.sites.every((s) => s.platformWebSocket && s.platformWebSocket.readyState === 1)
          const plazaWs = o.sites.every((s) => s.plazaWebSocket && s.plazaWebSocket.readyState === 1)
          const roomWs = o.sites.every((s) => s.roomWebSocket && s.roomWebSocket.readyState === 1)

          if (amount && amount >= o.totalLiushui && platformWs && plazaWs && roomWs) {
            setTimeout(() => {
    ...
    // 监控游戏局号. 新的局号诞生时下注
    gmcodeArr() {
      const gm = this.gmcodeArr[this.gmcodeArr.length - 1]
      const gmcode = gm.gmcode
      const schemeId = gm.schemeId
      const scheme = this.schemes.find((s) => s.schemeId === schemeId)
      const platformWs = scheme.sites.every((s) => s.platformWebSocket && s.platformWebSocket.readyState === 1)
      const plazaWs = scheme.sites.every((s) => s.plazaWebSocket && s.plazaWebSocket.readyState === 1)
      const roomWs = scheme.sites.every((s) => s.roomWebSocket && s.roomWebSocket.readyState === 1)
      if (scheme && scheme.isAllConnect && platformWs && plazaWs && roomWs) {
        if (scheme.amountType === 1) {
          const arr = scheme.randomRange.split('*').map(Number)
          ...
          const siteEven = scheme.sites.filter((s, i) => i % 2 === 0)
          const siteOdd = scheme.sites.filter((s, i) => i % 2 !== 0)
          ...
          siteEven.map((even) => {
            if (even.roomWebSocket && even.roomWebSocket.readyState === 1) {
              ...
              even.roomWebSocket.send(
                doSend(
                  cmd.gameBetExt(gmcode, type1, amount),
                  even.socketId,
                  even.userEncryptKey,
                  even.userDecryptKey,
              ...
              this.addWebSocketLog({
                ...
                msg: `${moment().locale('zh-cn').format('YYYY-MM-DD HH:mm:ss')}——网站：${even.name}——房间：${
                  even.vid
                }，局号：${gmcode}，下注：${type1 === 1 ? '庄' : '闲'}，金额：${amount}`
              })
            }
          })
          siteOdd.map((odd) => {
            if (odd.roomWebSocket && odd.roomWebSocket.readyState === 1) {
              ...
              odd.roomWebSocket.send(
                doSend(
                  cmd.gameBetExt(gmcode, type2, amount),
                  odd.socketId,
                  odd.userEncryptKey,
                  odd.userDecryptKey,
              ...
              this.addWebSocketLog({
                ...
                msg: `${moment().locale('zh-cn').format('YYYY-MM-DD HH:mm:ss')}——网站：${odd.name}——房间：${
                  odd.vid
                }，局号：${gmcode}，下注：${type2 === 1 ? '庄' : '闲'}，金额：${amount}`
              })
            }
          })
        } else if (scheme.amountType === 2) {
          const arr = scheme.randomValue.split('*').map(Number)
          ...
          const minArr = scheme.sites.find((s) => s.specifyValue.length === minLen).specifyValue
          ...
          scheme.sites.map((s) => {
            ...
            setTimeout(() => {
              s.roomWebSocket.send(
                doSend(
                  cmd.gameBetExt(gmcode, playType, amount),
                  s.socketId,
                  s.userEncryptKey,
                  s.userDecryptKey,
              ...
              this.addWebSocketLog({
                ...
                msg: `${moment().locale('zh-cn').format('YYYY-MM-DD HH:mm:ss')}——网站：${s.name}——房间：${
                  s.vid
                }，局号：${gmcode}，下注：${playType === 1 ? '庄' : '闲'}，金额：${amount}`
              })
    ...
    socket() {
      ...
      agInfo.map(async (agConfig) => {
        ...
        const result = await loginRoom(
          ...
          (gmcode) => {
            if (gmcode && !this.gmcodeArr.some((g) => g.gmcode === gmcode)) {
              this.gmcodeArr.push({ schemeId: schemeId, gmcode: gmcode })
            }
          },
          (userEncryptKey) => {
