...
const scheme = {
  ...
  mutations: {
    ...
    SCHEME_UPDATE(state, scheme) {
      if (state.schemeList) {
        const index = state.schemeList.findIndex((s) => s.schemeId === scheme.schemeId)
  ...
  actions: {
    ...
    async updateSchemeAsync({ commit }, scheme) {
      const res = await updateScheme(scheme)
      if (res.code === 0) {
        commit('SCHEME_UPDATE', scheme)
      }
      return res
    },
    updateScheme({ commit }, scheme) {
      commit('SCHEME_UPDATE', scheme)
    },
